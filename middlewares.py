...
class AminerspiderSpiderMiddleware(object):
...
class AminerspiderDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
...
class SeleniumAminerDownloaderMiddleware(object):
    ...
    def process_request(self, request, spider):
        if spider.name == "aminer":
            #request.replace('https://www.aminer.cn/search/pub?t=b&q=空间站&forceType=false')
            ...
            #res = Response(url=request.url, encoding='utf8',body=bytes(origin_code), request=request)
            return res
        if spider.name == 'bole':
            request.cookies = {}
